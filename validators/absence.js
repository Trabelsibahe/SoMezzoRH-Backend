...
const { isBefore, isAfter, differenceInCalendarDays } = require("date-fns");
...
module.exports = function ValidateAbsence(data, file) {
  ...
  if (!validator.isDate(data.dateDebut)) {
    errors.dateDebut = "Veuillez entrer la date de début de période d'absence.";
  }
  ...
  // Check the difference between dateDebut and dateFin
  if (
    ...
    !validator.isEmpty(data.dateDebut) &&
    !validator.isEmpty(data.dateFin) &&
    differenceInCalendarDays(new Date(data.dateFin), new Date(data.dateDebut)) > 15
  ) {
    errors.dateFin = "La période d'absence ne peut pas dépasser 15 jours.";
  }
